...
#                     comp_dict = zip(columns, (comp_data[:, i] for i in range(comp_data.shape[1])))
                    _comp_data_str = []
                    for j, row in enumerate(comp_data):
                        _comp_data_str.append(str(row))
#                     print('len comp_data', (comp_data))
                    data_dict.update({comp: _comp_data_str})
                else:
#                     print('flag 2 2')
                    data_dict.update({comp: comp_data})
            else:
#                 print('flag 3')
                data_dict.update({comp: comp_data})
#         print('data dict columns', data_dict.keys())
        _data = pd.DataFrame(data_dict, index=self.indices)
